...
/*add_action( 'plugins_loaded', function() {
    add_action( 'acf/init', function() {
        ...
        if ( in_array('events', $active_modules ) && taxonomy_exists( 'event-categories' ) ) {
            if ( register_taxonomy_for_object_type( 'event-categories', 'event_series' ) ) {
				error_log( 'Yay! Registered taxonomy event-categories for cpt event_series!' );
			} else {
				error_log( 'Uh oh! Failed to register taxonomy event-categories for cpt event_series!' );
			}
        }
    } );
} );*/
